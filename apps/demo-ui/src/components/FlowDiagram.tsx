...
export function FlowDiagram({ step }: Props) {
  ...
  const nodes = useMemo<Node[]>(() => BASE_NODES.map(n => {
    ...
    switch (n.id) {
      ...
      case "relay_b":    return { ...n, data: { label: "Relay B",        domain: "company-a.aamp",          badge: "Intra",     provider: "CF",      scheme: "cyan",   active } };
      case "research":   return { ...n, data: { label: "Research Bot",   role: "Company A · PDF Analyst",   tech: "Python",     provider: "Intra",   scheme: "blue",   active } };
      case "relay_c":    return { ...n, data: { label: "Relay C",        domain: "company-b.sandbox",       badge: "Inter",     provider: "Vercel",  scheme: "amber",  active } };
      case "compliance": return { ...n, data: { label: "Compliance Bot", role: "Company B · Reg Checker",   tech: "TypeScript", provider: "Inter",   scheme: "amber",  active } };
      default: return n;
    }
  ...
  const stepMeta: Record<FlowStep, { label: string; variant: "idle" | "active" | "done" | "error" }> = {
    ...
    probing:                   { label: "PROBE → Research Bot [Company A · Intra]",                variant: "active" },
    probe_sent:                { label: "PROBE dispatched → Relay A [Company A · Intra]",          variant: "active" },
    probe_federated:           { label: "PROBE: Relay A → Relay B via NATS [Company A · Intra]",   variant: "active" },
    probe_delivered:           { label: "PROBE delivered to Research Bot [Company A]",             variant: "active" },
    probe_response:            { label: "PROBE_RESPONSE ✓ — returning within Company A",          variant: "active" },
    task_sent:                 { label: "TASK dispatched → Relay A [Company A · Intra]",           variant: "active" },
    task_federated:            { label: "TASK: Relay A → Relay B via NATS [Company A · Intra]",    variant: "active" },
    task_delivered:            { label: "TASK delivered to Research Bot [Company A]",              variant: "active" },
    task_processing:           { label: "Research Bot processing document… [Company A]",            variant: "active" },
    task_response:             { label: "RESPONSE flowing back within Company A",                  variant: "active" },
    research_done:             { label: "✓ Company A (Intra) done — starting Company B (Inter)",   variant: "active" },
    compliance_probing:        { label: "PROBE → Compliance Bot [Company B · Inter]",              variant: "active" },
    compliance_probe_response: { label: "PROBE_RESPONSE ✓ — Relay C accepted [Company B]",        variant: "active" },
    compliance_task_sent:      { label: "Compliance-check TASK → Relay C [Company B · Inter]",     variant: "active" },
    compliance_processing:     { label: "Compliance Bot running checks… [Company B · Vercel SB]",  variant: "active" },
    compliance_done:           { label: "✓ Company B (Inter) done — cross-company flow complete!", variant: "done"   },
    completed:                 { label: "✓ Company A (Intra) + Company B (Inter) complete",        variant: "done"   },
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden border border-white/[0.07] bg-zinc-950" style={{ height: 380 }}>
      ...
      <div className="absolute inset-0 pointer-events-none select-none" style={{ zIndex: 0 }}>
        {/* Row 1 — Company A (Intra): Finance Bot + Relay A + Relay B + Research Bot */}
        <div className="absolute left-0 right-0 border-b border-white/[0.04]" style={{ top: 0, height: "53%" }}>
          <div className="absolute inset-0 bg-blue-950/10 flex items-end pb-2 pl-3">
            <span className="text-[7px] font-bold uppercase tracking-[0.2em] text-blue-500/40">Company A — Intra-Provider (Cloudflare Workers)</span>
          </div>
        </div>
        {/* Row 2 — Company B (Inter): Relay C + Compliance Bot */}
        <div className="absolute left-0 right-[40%] bottom-0" style={{ top: "53%" }}>
          <div className="absolute inset-0 bg-amber-950/10 border-r border-amber-500/10 flex items-center pb-2 pl-3">
            <span className="text-[7px] font-bold uppercase tracking-[0.2em] text-amber-500/40">Company B — Inter-Provider (Vercel Sandbox)</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <Panel position="bottom-right" style={{ zIndex: 10 }}>
          <div className="mb-2 mr-2 bg-zinc-950/80 backdrop-blur-sm border border-white/[0.07] rounded-xl px-3 py-2 flex flex-col gap-1.5">
            ...
            <LegendRow color={P1_FWD} label="Company A request (Intra)" />
            <LegendRow color={P1_RET} label="Company A response (Intra)" />
            <LegendRow color={P2_FWD} label="→ Company B forward (Inter)" />
            <LegendRow color={P2_RET} label="← Company B return (Inter)" />
          </div>
        </Panel>
      </ReactFlow>
    </div>
